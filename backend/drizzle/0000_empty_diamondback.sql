...
CREATE TYPE "public"."budget_context" AS ENUM('individual', 'joint');--> statement-breakpoint
...
CREATE TYPE "public"."notification_type" AS ENUM('bill_reminder', 'goal_completed', 'goal_near_completion', 'budget_alert', 'couple_invite', 'general');--> statement-breakpoint
...
CREATE TABLE IF NOT EXISTS "monthly_budgets" (
	...
	"context" "budget_context" DEFAULT 'individual' NOT NULL,
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "monthly_budgets" ADD CONSTRAINT "monthly_budgets_couple_id_couples_id_fk" FOREIGN KEY ("couple_id") REFERENCES "public"."couples"("id") ON DELETE set null ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
